...
bambu -O3 -v5 --std=c11 --device-name=$DEVICE   --top-fname=main	\
	--backend-sdc-extensions=$BOARD/master.sdc 						\
	--backend-script-extensions=gig_eth_pcs_pma_v11.inc				\
	--clock-period=$CLK_PERIOD 										\
	--file-input-data=${BASE}/src/microserver.v,${BASE}/src/driver.v,${BASE}/src/clock-arch.v --evaluation --objective=PERIOD,AREA,FREQUENCY,CLOCK_SLACK,REGISTERS,DSPS,BRAMS	\
	-I${BASE}/src/						\
	-I${UCIP}/uip/						\
